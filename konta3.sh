...
do
	...
	HASLO=`echo $I | cut -d":" -f5` 		# wyłuskanie hasla
	...
	psql -d postfixadmin -U postfixadmin -c 'INSERT INTO mailbox (local_part,username,name,password,maildir,domain,active,quota) VALUES ('$LOCAL','${LOCAL}@${DOMENA}','$NAZWA','$HASLO','${DOMENA}/${LOCAL}@${DOMENA}${maildir}','$DOMENA','t','$QUOTA');'
	

	psql -d postfixadmin -U postfixadmin -c 'INSERT INTO alias (address,goto,domain,active) VALUES ('${LOCAL}@${DOMENA}','${LOCAL}@${DOMENA}','$DOMENA','t');'
	
	psql -d postfixadmin -U postfixadmin -c 'INSERT INTO alias (address,goto,domain,active) VALUES ('${ALIAS}@${DOMENA}','${LOCAL}@${DOMENA}','$DOMENA','t');'
